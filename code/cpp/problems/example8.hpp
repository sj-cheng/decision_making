...
class Example8 : public Problem { 
	...
	public:
		...
		// 	r.setZero();
		// 	if (is_captured(next_state) || next_state(4,0) >= m_tf){
		// 		float t = std::min(next_state(4,0), m_tf);
		// 		r(0,0) = t / m_tf;
		// 		r(1,0) = 1.0 - r(0,0);
		// 	} else if ( !(
		// 		(next_state.block(0,0,2,1).array() >= m_state_lims.block(0,0,2,1).array()).all() && 
		// 		(next_state.block(0,0,2,1).array() <= m_state_lims.block(0,1,2,1).array()).all() )) {
		// 		r(0,0) = -1.0;
		// 	} else if ( !(
		// 		(next_state.block(2,0,2,1).array() >= m_state_lims.block(2,0,2,1).array()).all() &&
		// 		(next_state.block(2,0,2,1).array() <= m_state_lims.block(2,1,2,1).array()).all() )) {
		// 		r(1,0) = -1.0;
		// 	}
        //     return r;
        // }
